<?php
       ...
       $aColumns = array( 'idclients', 'lastName', '' , 'tel', 'email', 'www', 'idclients' );
		...
		for ( $i=0 ; $i<intval( $_GET['iSortingCols'] ) ; $i++ )
		{
			if ( $_GET[ 'bSortable_'.intval($_GET['iSortCol_'.$i]) ] == "true" )
			{
                            if ($_GET['iSortCol_'.$i] == '1')
                                $q->addOrderBy('CONCAT(c.lastName, c.name) '.$_GET['sSortDir_'.$i]); else
                            if ($_GET['iSortCol_'.$i] == '2')
                                $q->addOrderBy('a.city '.$_GET['sSortDir_'.$i]); else
